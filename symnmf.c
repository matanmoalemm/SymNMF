...
/*Reads the first line of a data file to determine the dimension d.
This function sets the global variable d, representing the dimensionality.*/
...
/*Reads data points from a file and stores them in a dynamically allocated array.
 Calls compute_d to set the dimension and uses linked lists for initial storage,
 converting them to a 2D array. This function sets a global variable d*/
...
/*Handles memory allocation failure by printing an error message and terminating the program*/
...
/*Constructs a symmetric matrix A from the data points.*/
...
/*Creates a degree diagonal matrix D from matrix A, with diagonal entries as row sums of A.*/
double** ddg(double **A){
    int i,j;
    double sum;
...
/*Creates a normalized matrix W by calculating D^0.5*A*D^0.5*/
...
/*Checks convergence by comparing the squared differences of corresponding elements in prevH and newH.*/
...
/*Performs one iteration of matrix H update using W, following the algorithm's update rule.*/
...
/*Repeatedly updates matrix H until convergence or until maximum iterations are reached.*/
...
/*Creates a deep copy of matrix H.*/
...
/*Allocates and initializes a 2D array of doubles with the specified dimensions.*/
